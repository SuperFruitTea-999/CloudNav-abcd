          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-1 scrollbar-hide">
              <button
                onClick={() => {
                  if (selectedCategory === 'all' && !searchQuery) {
                    // 如果已经在首页且没有搜索，点击则回滚到顶部
                    const scrollArea = document.querySelector('.overflow-y-auto.p-4.lg\\:p-8');
                    if (scrollArea) {
              ...
              {(!searchQuery && selectedCategory === 'all') ? (
                ...
                <div className="space-y-12">
                  {categories.map(cat => {
                    ...
                    // 如果分类没有链接且不是锁定的，则跳过不显示
                    if (catLinks.length === 0 && !isLocked) return null;
                        ...
                        {collapsedCategoryIds.has(cat.id) ? null : (isLocked ? (
                          <div className="flex flex-col items-center justify-center py-10 text-slate-400 bg-slate-100/50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                            ...
                            <button
                              ...
                              className="mt-3 px-4 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-medium rounded-lg transition-colors"
                            >
                              输入密码解锁
                            </button>
                          </div>
